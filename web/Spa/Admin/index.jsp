<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!doctype html>
<html lang="en">
<head>
<%@include file="Layout/styles.jsp"%>
<title>Title</title>
</head>
<body>

	<%@include file="Layout/header.jsp"%>
	<div class="container-fluid">
		<div class="row">
			<%@include file="Layout/navigation.jsp"%>
			<main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
			<div
				class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
				<h1 class="h2">Dashboard</h1>


			</div>

			<!--Main content-->
			<div class="col pb-2 mb-3">

				<div class="row">


					<div class="container">
						<div class="card-deck">
							<div class="card">
								<img class="card-img-top"
									src="https://portail-animation.ufcv.fr/tap-cintre/wp-content/uploads/sites/120/2017/12/relaxante.jpg"
									alt="Card image cap">
								<div class="card-body">
									<h3 class="card-title">Olu Spa</h3>
									<p class="card-text">Address - Duch hospital Rd,Gall</p>
									<p class="card-text">Phone - 0914537890</p>

								</div>
							</div>
							<div class="card">
								<img class="card-img-top" src="../Images/12224761.jpg" alt="Spa Lawanga">
								<div class="card-body">
									<h3 class="Card-title">Spa Lawanga</h3>
									<p class="card-text">Address - Gall Rd,Hikkaduwa</p>
									<p class="card-text">Phone - 0112435674</p>
								</div>
							</div>
							<div class="card">
								<img class="card-img-top"
									src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSXtWtv9o5HH6JZ7ECDjVkAqsuG34Vof6RLLQxPUPgZbpqR0oKNDA"
									alt="Card image cap">
								<div class="card-body">
									<h3 class="card-title">Paradise garden Spa</h3>
									<p class="card-text">Address - 95/B Yaddehimulla Rd
										Unawatuna</p>
									<p class="card-text">Phone - 01197867543</p>
								</div>
							</div>
						</div>
					</div>
				</div>


				<div class="row pt-5">
					<div class="container">
						<div class="card-deck">
							<div class="card">
								<img class="card-img-top"
									src="data:image/jpeg;base64,"
									alt="Card image cap">
								<div class="card-body">
									<h3 class="card-title">Amaya spa</h3>
									<p class="card-text">Address East Tower world Trade Center
										,colombo</p>
									<p class="card-text">0654374200</p>
								</div>
							</div>
							<div class="card">
									<img class="card-img-top" src="https://i.pinimg.com/originals/d9/f7/0d/d9f70dd171b2708025d5e450493289aa.jpg"
									alt="Card image cap">
								<div class="card-body">
									<h3 class="card-title">Thaproban pavillion Spa</h3>
									<p class="card-text">Address - unawatuna</p>
									<p class="card-text">Phone - 0914944185</p>
								</div>
							</div>
							<div class="card">
								<img class="card-img-top"
									src="https://a-static.besthdwallpaper.com/spa-wallpaper-2400_L.jpg"
									alt="Card image cap">
								<div class="card-body">
									<div class="card-body">
										<h3 class="card-title">Ayurveda Lanka Spa</h3>
										<p class="card-text">Address - No 6A, Dewala Rd, unawatuna</p>
										<p class="card-text">Phone - 0771764662</p>
									</div>
								</div>
							</div>
						</div>
					</div>


				</div>


			</div>
			<!--End of main content--> </main>
		</div>
	</div>
	<%@include file="Layout/footer.jsp"%>
	<%@include file="Layout/scripts.jsp"%>
</body>
</html>